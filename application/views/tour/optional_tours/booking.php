		...
		<div class="row">
			<div class="col-sm-8">
				...
				<div class="price-table">
					...
					<table class="table table-bordered">
						<tbody>
							<tr style="background:#c5030b;" class="tbl-heading">
										...
										if ($hassupplement) {
											...
											foreach ($rates as $rate) {
												if ($rate->type_passenger == $valtype && $rate->single_supplement) {
													echo '<td class="text-center"><span class="price font-weight-semibold">+'.number_format($rate->price,2,'.',',').' <span class="unit font-weight-semibold">USD</span></td>';
													$hasrate = 1;
													unset($rate);
													break;
												}
			...
			<div class="col-sm-4">
				<div class="box-transfer">
					<form action="<?=site_url('tours/review')?>" method="POST">
						<div class="heading p-3"><?=$item->title;?></div>
						<div class="p-3">
							<div class="select-date mb-3">
								<div class="title">Select Date</div>
								<input id="datepicker" width="100%" name="arrival_date" value=""/>
								<script>
							...
							<div class="passenger mb-3">
									...
									<div class="box-passenger">
										<div class="type-passenger clearfix">
											...
											<div class="touchspin float-right">
												...
												<div class="passenger-amount">
													<input name="adults" type="text" class="amount-adults"  value="1">
												</div>
												<div class="plus-btn">
										...
										<div class="type-passenger clearfix">
											...
											<div class="touchspin float-right">
												...
												<div class="passenger-amount">
													<input name="childs" type="text" class="amount-childs" value="0">
												</div>
												<div class="plus-btn">
										...
										<div class="type-passenger clearfix">
											...
											<div class="touchspin float-right">
												...
												<div class="passenger-amount">
													<input name="infants" type="text" class="amount-infants" value="0">
												</div>
												<div class="plus-btn">
								...
								<script>
									...
									$('.operation').click(function(event) {
										a = parseFloat($(this).parents('.touchspin').find('.passenger-amount > input').val());
										typ = $(this).attr('typ');
										if($(this).html() == '+') {
											$(this).parents('.touchspin').find('.passenger-amount > input').val(a+1);
										} else {
									...
									function get_tour_fee() {
										var p = {};
										p['adult'] = parseInt($('.amount-adults').val());
										p['child'] = parseInt($('.amount-childs').val());
										p['infant'] = parseInt($('.amount-infants').val());
										p['id'] = <?=$item->id;?>;

										$.ajax({
											url: '<?=site_url("tours/ajax-api");?>',
											type: 'post',
											dataType: 'json',
											data: p,
											success: function(result) {
												$('.total').html(result.toFixed(0));
											},
											error: function(err){
												console.log(err);
											}
								...
								<div class="total-price mb-3 clearfix">
									<div class="title float-left">Total price</div>
									<div class="float-right" style="font-size: 24px; font-weight: 600; color: #ed1c24;"><span class="total"></span>USD</div>
								</div>
								<button type="submit" class="btn btn-success w-100 btn-review py-2 text-white font-weight-bold mb-3">PROCEED TO PAYMENT</button>
							<div class="text-center">
								<div>We accept online payment with</div>
								<img src="<?=IMG_URL?>payment-methods.jpg" alt="">
