		...
		<form id="frm-admin" name="adminForm" action="" method="POST">
			...
			<ul class="nav nav-tabs tab-agent" role="tablist">
				...
				<li role="presentation" class="<?=($agents_id == $agent->id) ? 'active' : ''?>">
					<a href="<?=site_url("syslog/debt-approved-list/{$agent->id}")?>">
						<h5><?=$agent->name?></h5>
						<p>(<?=$agent->company?>)</p>
					</a>
				</li>
					...
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane active" id="for-tourist">
							<table id="export-list" class="table table-bordered table-hover">
								...
								foreach ($items as $item) {
									if (($item->visit_purpose == 'For tourist') && ($item->send_approved == 2)) {
								?>
								<tr class="row1 prss0">
									...
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
										<?
											if ($item->note_fc == null || $item->note_fc == 0) {
												if ((date('Y-m-d',strtotime($item->arrival_date)) >= date('Y-m-d')) && (date('Y-m-d',strtotime($item->arrival_date)) <= date('Y-m-d',strtotime('+ 2days')))){
									...
									<?
										...
										$info = new stdClass();
										$info->nation_id = $item->nation_id;
										$info->agents_id = $item->agents_id;
										$fee_visa = $this->m_agent_visa_fee->items($info);
										$visa_fee += $fee_visa[0]->{$type};
										if (!empty($processing)) {
											$processing_fee = $this->m_agent_processing_fee->item($item->agents_id,$item->nation_type);
											$visa_fee += $processing_fee->{$processing};
										}
										if (!empty($item->private_visa)) {
											$info = new stdClass();
											$info->agents_id = $item->agents_id;
											$private_letter_fee = $this->m_agent_private_letter_fee->items($info);
											$visa_fee += $private_letter_fee[0]->{$type};
										}
										if ((date('Y-m-d',strtotime($item->arrival_date)) >= date('Y-m-d')) && (date('Y-m-d',strtotime($item->arrival_date)) <= date('Y-m-d',strtotime('+ 2days')))){
											$info = new stdClass();
										...
										$total_visa_fee += $visa_fee;
										$total_fc_fee += $fc_fee;
										$total_stamp_fee += $stamp_fee;
										$total_car_fee += $car_fee;
										$total_plus_fee += $item->plus_fee;
										?>
									...
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center">
										<input type="text" class="note-input" name="plus_fee" value="<?=$item->plus_fee?>" pro-type="plus_fee" old-val="<?=$item->plus_fee?>" typ="t" item-id="<?=$item->id?>" style="border:none;text-align: center;width: 40px;">
									</td>
								</tr>
								<? $j++; } } ?>
								<tr>
									<td colspan="16" style="text-align: center;"><strong>Total: $</strong><strong class="t-total-fee"><?=($total_visa_fee + $total_fc_fee + $total_stamp_fee + $total_car_fee + $total_plus_fee)?></strong></td>
									<td style="text-align: center;"><strong>$<?=$total_visa_fee?></strong></td>
									...
									<td style="text-align: center;"><strong>$</strong><strong class="t-total-plus-fee"><?=$total_plus_fee?></strong></td>
								</tr>
							</table>
						</div>
						<div role="tabpanel" class="tab-pane" id="for-business">
							<table id="export-list" class="table table-bordered table-hover">
								...
								$total_plus_fee = 0;
								foreach ($items as $item) { 
									if ($item->visit_purpose == 'For business' && ($item->send_approved == 2)) {
								?>
								<tr class="row1 prss0">
									<td style="vertical-align: middle;border-right: thin solid #999;"><?=$j+1;?></td>
									<td style="vertical-align: middle;border-right: thin solid #999;"><?=BOOKING_PREFIX.$item->book_id?></td>
									...
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center">
										<input type="text" class="note-input" name="plus_fee" value="<?=$item->plus_fee?>" pro-type="plus_fee" typ="b" old-val="<?=$item->plus_fee?>" item-id="<?=$item->id?>" style="border:none;text-align: center;width: 40px;">
									</td>
								</tr>
								<? $j++; } } ?>
								<tr>
									<td colspan="16" style="text-align: center;"><strong>Total: $</strong><strong class="b-total-fee"><?=($total_visa_fee + $total_fc_fee + $total_stamp_fee + $total_car_fee + $total_plus_fee)?></strong></td>
									<td style="text-align: center;"><strong>$<?=$total_visa_fee?></strong></td>
									<td style="text-align: center;"><strong>$<?=$total_fc_fee?></strong></td>
									<td style="text-align: center;"><strong>$<?=$total_stamp_fee?></strong></td>
									<td style="text-align: center;"><strong>$<?=$total_car_fee?></strong></td>
									<td style="text-align: center;"><strong>$</strong><strong class="b-total-plus-fee"><?=$total_plus_fee?></strong></td>
								</tr>
							</table>
						</div>
					</div>
...
<script>
	...
	$(document).on('change', '.note-input', function(event) {
		...
		$('.'+typ+'-total-plus-fee').html(tpf+result);
		$('.'+typ+'-total-fee').html(tf+result);
