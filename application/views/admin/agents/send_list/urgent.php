	...
	<div class="container-fluid">
				...
				<div class="pull-right">
					<div class="clearfix">
						<div class="pull-left" style="max-width: 320px; margin-right: 5px;">
							...
							<button type="button" class="btn btn-success" data-toggle="modal" data-target="#myModal">Add booking</button>
						</div>
					</div>
				</div>
			</h1>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">Booking ID</h4>
					</div>
					<div class="modal-body">
						<input type="text" name="booking_id" id="booking_id" class="form-control" value="">
						<div class="radio">
							<label>
								<input type="radio" name="type" value="urgent" checked="checked">
								Send Urgent
							</label>
							<label>
								<input type="radio" name="type" value="again">
								Send Again
							</label>
						</div>
					</div>
		...
		<script type="text/javascript">
			$('.btn-add').click(function(event) {
				var book_id = $('#booking_id').val();
				
				var p = {};
				p["book_id"] = book_id;
				p["type"] = $('input[name=type]:checked').val();
				$.ajax({
					type: "POST",
					url: "<?=site_url("syslog/ajax-add-urgent-mail")?>",
					data: p,
		...
		<form id="frm-admin" name="adminForm" action="" method="POST">
			...
			<div class="table-responsive" style="font-size: 12px;">
				<table id="export-list" class="table table-bordered table-hover">
					<tr>
						<th class="text-center" colspan="19">For tourist</th>
					</tr>
					<tr>
						<th style="border-right: thin solid #999;background: #009e00;" width="40px">
							No.
						</th>
					...
					<tr class="row1 prss0">
						...
						<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
							<label style="cursor: pointer;">
								<i class="fa fa-upload" data-toggle="modal" data-target="#myModal1" pax_id="<?=$item->pax_id?>"></i>
							</label>
						</td>
						<td style="vertical-align: middle;border-right: thin solid #999;">
						...
						<td class="text-center">
							<a><i style="cursor: pointer;" item-id="<?=$item->pax_id?>" class="fa fa-trash-o" type="<?=!empty($item->send_again) ? 'send_again' : 'send_urgent'?>" aria-hidden="true"></i></a>
						</td>
					</tr>
					<? $j++; } } ?>
				</table>
				<table id="export-list" class="table table-bordered table-hover">
					<tr>
						<th class="text-center" colspan="19">For business</th>
					</tr>
					<tr>
						<th style="border-right: thin solid #999;background: #009e00;" width="40px">
							No.
...
<script type="text/javascript">
	$('.fa-upload').click(function(event) {
		...
		p['id'] = pax_id;
		$.ajax({
			url : '<?=BASE_URL.'/syslog/ajax-get-attach-file-urg'?>',
			type : 'POST',
			data : p,
			dataType: 'json',
			success : function(data) {
				if (data) {
					...
					var str = '';
					for (var i = 0; i < c; i++) {
						var src = '<?=BASE_URL.'/files/upload/image/passport_photo_urg/'?>'+pax_id+'/'+data[i];
	...
	$(document).on('click', '.btn-delete', function(event) {
		...
		$.ajax({
			url : '<?=BASE_URL.'/syslog/ajax-delete-attach-file-urg'?>',
			type : 'POST',
			data : p,
			dataType: 'json',
			success : function(data) {
	...
	$('.fa-trash-o').click(function(event) {
		...
		if (cf) {
			...
			$.ajax({
				type: "POST",
				url: "<?=site_url("syslog/ajax-del-urgent-mail")?>",
				data: p,
				success: function (result) {
					location.reload();
				}
