			...
			<h1 class="page-title">
				<div class="pull-right">
					<ul class="action-icon-list">
						...
						<li><a href="<?=site_url("syslog/tour/{$categories_id}/add")?>"><i class="fa fa-plus" aria-hidden="true"></i> Add</a></li>
					</ul>
				</div>
			</h1>
		</div>
			...
			<table class="table table-bordered table-hover">
				...
				<?
					...
					foreach ($items as $item) {
						$histories = $this->m_history->search($this->m_tour->_table, $item->id, NULL, 5);
				...
				<tr class="row<?=$item->active?>">
					...
					<td>
						<a href="<?=site_url("syslog/tour/{$item->catid}/edit/{$item->id}")?>"><?=$item->title?></a>
						<ul class="action-icon-list">
							<li><a href="<?=site_url("syslog/tour/edit/{$item->id}")?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a></li>
					...
					<?
						...
						$count_rate = $this->m_tour_rate->items($info);
					?>
					<td><a href="<?=site_url("{$this->util->slug($this->router->fetch_class())}/tour-rates/{$item->id}")?>"><?=count($count_rate);?></a></td>
					<td class="hidden">
						<?
						/*
