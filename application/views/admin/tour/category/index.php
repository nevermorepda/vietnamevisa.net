			...
			<h1 class="page-title">
				...
				<div class="pull-right">
					<ul class="action-icon-list">
						...
						<li><a href="<?=site_url("syslog/tour-categories/add")?>"><i class="fa fa-plus" aria-hidden="true"></i> Add</a></li>
					</ul>
				</div>
			</h1>
		</div>
					...
					function print_categories($obj, $categories, $level, &$i) {
						foreach ($categories as $category) {
							...
							<tr class="row<?=$category->active?>">
								...
								<td>
									<a href="<?=site_url("syslog/tour-categories/edit/{$category->id}")?>"><?=level_indent($level).($level?"|&rarr; ":"")?><?=$category->name?></a>
									<ul class="action-icon-list">
										<li><a href="<?=site_url("syslog/tour/{$category->id}")?>"><?=number_format($category->child_num)?> Topics</a></li>
										<li><a href="<?=site_url("syslog/tour-categories/edit/{$category->id}")?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a></li>
