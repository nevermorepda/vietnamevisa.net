			...
			<h1 class="page-title">
				...
				<div class="pull-right">
					<ul class="action-icon-list">
						...
						<li><a href="<?=site_url("syslog/useful/{$category->id}/add")?>"><i class="fa fa-plus" aria-hidden="true"></i> Add</a></li>
					</ul>
				</div>
			</h1>
		</div>
		...
		<form id="frm-admin" name="adminForm" action="" method="POST">
			...
			<table class="table table-bordered">
				...
				<tr class="row<?=$item->active?>">
					...
					<td>
						<a href="<?=site_url("syslog/useful/{$category->id}/edit/{$item->id}")?>"><?=$item->title?></a>
						<ul class="action-icon-list">
							<li><?=number_format($item->view_num)?> Views</li>
							<li><a href="<?=site_url("syslog/useful/{$category->id}/edit/{$item->id}")?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a></li>
