		...
		if (!empty($item->client_ip)) {
			if ($item->status) {
				if (array_key_exists($item->country_name, $countries)) {
					...
					if (date("Y-m-d", strtotime($item->user_registered)) < date("Y-m-d", strtotime($date_remind))) {
...
<div class="cluster">
	<div class="container-fluid">
				...
				<div class="pull-right">
					<div class="clearfix">
						...
						<div class="pull-left" style="max-width: 220px;">
							<div class="input-group input-group-sm">
								<input type="date" id="date" class="form-control" value="<?=date('Y-m-d',strtotime("$date_remind"))?>">
		...
		<form id="frm-admin" name="adminForm" action="<?=site_url("syslog/mail-remind")?>" method="POST">
			...
			<input type="hidden" id="date_remind" name="date_remind" value="<?=$date_remind?>" />
										...
										<div class="row">
											<div class="col-sm-7">
												...
												for ($i=0; $i<$item->group_size; $i++) {
													...
													$trl_lines .= '<tr><td class="text-center" '.$style.'>'.($i+1).'</td><td '.$style.'>'.$travelers[$i]->fullname.'</td><td class="text-center" '.$style.'>'.$travelers[$i]->gender.'</td><td class="text-center" '.$style.'>'.date("M/d/Y", strtotime($travelers[$i]->birthday)).'</td><td class="text-center" '.$style.'>'.$travelers[$i]->nationality.'</td><td class="text-center" '.$style.'>'.$travelers[$i]->passport.'</td><td class="text-center" '.$style.'><a class="pointer btn-edit-passport" item-id="'.$travelers[$i]->id.'"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a></td></tr>';
												}
